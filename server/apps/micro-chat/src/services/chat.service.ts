...
return {personalChats,groupChats}
  ...
  async savePersonalMessage(user:JwtAuthGuardTrueType, createMessage:CreatePersonalMessageDto){
    ...
    if(!createMessage.textContent && !createMessage.mediaContent ){
      throw new WsException({status:MessageStatusEnum.BAD_REQUEST,message:"Text content or media is required"});
    }
    const sentAt = createMessage.sentAt || new Date().toISOString();
    const message = new Message({
      id:createMessage.id,
      userId:user.userId,
      chatId:createMessage.chatId,
      mediaType:createMessage.mediaType,
      textContent:createMessage.textContent,
      mediaContent:createMessage.mediaContent,   
      sentAt
    })
    await this.entityManager.save(message);
    return message
  }
  async updateMessageDelivered(user:JwtAuthGuardTrueType, messageDeliveredDto:MessageDeliveredDto){
    await this.entityManager.transaction(async tEM=>{
      const messages = await tEM.find(Message,{
        where:{
          id:In(messageDeliveredDto.messageIds),
        }
      })
      messages.forEach(message=>{
        message.deliveredAt = messageDeliveredDto.messageIds.find(m=>m.messageId===message.id)?.deliveredAt
      })
      await tEM.save(messages)
    })
      
  }
  async updateMessageSeen(user:JwtAuthGuardTrueType, messageSeenDto:MessageSeenDto){
  
    await this.entityManager.transaction(async tEM=>{
      const messages = await tEM.find(Message,{
        where:{
          id:In(messageSeenDto.messageIds.map(m=>m.messageId)),
        }
      })
       messageSeenDto.messageIds.forEach( message=>{
        const messageExist = messages.find(m=>m.id===message.messageId)
        if(messageExist){
          messageExist.seenAt = message.seenAt
        }
      })
      await tEM.save(messages)
    })
  ...
  async checkUserOnChatSection(user:JwtAuthGuardTrueType){
    const userExist = await this.getUserFromDb(user.userId);
    if(userExist){
      return true
    }
    return false
